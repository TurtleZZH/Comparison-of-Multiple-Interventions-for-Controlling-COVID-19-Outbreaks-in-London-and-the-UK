...
%--------------------------------------------------------------------------
clear;clc;
%-----------------------武汉市进行强干预措施 Wuhan Suppression intervetion-------------------------------
%--------------------------------------------------------------------------
%   参数设置 set parameters
%--------------------------------------------------------------------------
...
W = 0;                                                                     %每日新增感染人数 New infections per day
H = 0;                                                                     %当前的医疗资源数 Current medical resources
...
y = 1/5;                                                                 %潜伏者自愈率 recovered rate of exposed
...
yhc = 1/14;                                                                %医院内中重症康复时间中位数13天 median recovery time for severe cases is 13 days in hospital
yac = 1/21;                                                                %医院内老龄中重症康复时间18天 recovery time of aged patients with serious diseases is 18 days in hospital
...
dhc = 1/28;                                                                %医院内中重症死亡中位数21天 Median number of death in hospital is 21 days
...
for idx = 1:length(T)-1
    ...
    elseif idx>= 43
        hospital = 7000;
        h = h +0.01;
    elseif idx>=32                                                         %以十二月二十二日为起准，32日后即一月二十三日进行限制流通措施 from December 22, restrictions was imposed for 32 days
        i = 0.1;
        h = h +0.01;
        hospital = hospital + 200;
        if r2(idx) ~= 3                                                    %强干预下，流通限制强从15到3，采取限制后每天减少两个人 Under the strong intervention, the circulation restriction was  from 15 to 3, and the restriction was reduced by 2 person per day
            r2(idx+1) = r2(idx)-2;
        end
    elseif idx>=28
        j = 0.5;
    ...
    W(idx+1) = r*B*S(idx)*(M(idx)+C(idx))/N(1) + i*r2(idx)*B2*S(idx)*E(idx)/N;
    %The number of available beds(Wuhan)
    H(idx+1) = hospital*h*j;
    ...
    %The number of Non-Hospitals(Wuhan)
    hospital_out1(idx+1) = hospital_out(idx);
    %The number of demand beds(Wuhan)
    C1(idx+1) = C(idx);
...
plot(T,hospital_out1,T,H,T,C1);grid on;hold on;
legend('The number of Non-Hospitals(Wuhan)','The number of available beds(Wuhan)','The number of demand beds(Wuhan)')
...
hold on;
title('Wuhan -- Suppression Intervetion on 23rd January')