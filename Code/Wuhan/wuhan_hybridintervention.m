...
%--------------------------------------------------------------------------
clear;clc;
%-----------------------武汉市进行放缩干预措施 Wuhan Hybrid intervetion----
%--------------------------------------------------------------------------
%   参数设置 set parameters
%--------------------------------------------------------------------------
...
for idx = 1:length(T)-1
    ...
    elseif idx>= 43
        hospital = 7000;
        h = h +0.01;
    elseif idx>=32                                                         %以十二月二十二日为起准，32日后即一月二十三日进行缩放限制流通措施 Based on December 22nd, 32 days later, January 23rd, we will implement measures to restrict circulation
        h = h +0.01;
        hospital = hospital + 200;
        if r2(idx) ~= 3                                                    %缩放干预下，流通限制强从15到3，采取限制后每天减少两个人 % With scaled intervention, the circulation limit is strong from 15 to 3, and two persons are reduced every day after taking the limit
            r2(idx+1) = r2(idx)-2;
        end
    elseif idx>=28
        j = 0.5;
...
hold on;
title('Wuhan -- Hybrid Intervetion on 23rd January')