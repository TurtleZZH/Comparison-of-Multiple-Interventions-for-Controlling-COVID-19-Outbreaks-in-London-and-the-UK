...
%--------------------------------------------------------------------------
clear;clc;
%-------------------伦敦地区和非伦敦地区进行适中的流通干预措施 intervetion in London and non-London-------------
%--------------------------------------------------------------------------
%   参数设置 set parameters
%--------------------------------------------------------------------------
%---伦敦--- London
...
Q2 = 0;                                                                    %总感染人数(英国总计) total infected
...
for idx = 1:length(T)-1
    r2(idx+1) = r2(idx);
    r3(idx+1) = r3(idx);
    if idx >= 46                                                           %以二月七日为起准，46日后即三月二十三日进行适当的干预流通措施 from February 7, restrictions was imposed for 46 days
        if r2(idx) ~= 6                                                    %强干预下，流通限制强从10到6，采取限制后每天减少1个人  Under the strong intervention, the circulation restriction was  from 10 to 3, and the restriction was reduced by 1 person per day
            r2(idx+1) = r2(idx)-1;
            r3(idx+1) = r3(idx)-1;
        end
        if h <= 0.6
    ...
    elseif idx>=35                                                         %以二月七日为起准，35日后即三月一十二日进行一定的流通限制措施 from February 7, restrictions on circulation was imposed on for 35 days
        ...
        if r2(idx) ~= 10                                                   %一定的流通限制干预下，流通限制强从15到10，采取限制后每天减少1个人 With interventions, circulation restrictions were from 15 to 10, with 1 less people per day after the restriction
            r2(idx+1) = r2(idx)-1;
        end   
        if r3(idx) ~= 10                                                   %一定的流通限制干预下，流通限制强从12到10，采取限制后每天减少1个人 Under intervention, circulation restriction was from 12 to 10, with 1 less person per day after the restriction
            r3(idx+1) = r3(idx)-1;
        end 
    end
    %---伦敦--- London
...
title('UK -- Mitigation Intervetion on 23rd March')